...
// User assigns one or more existing tags to selected contacts
// Uses Set collection to manage tag assignments
// @author Developer
// @version 12.0
...
public class Main {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			System.out.println("12. Apply Tags");
			...
			case 12:
				...
				Contact selectedContact = contactlist.get(contactIndex);
				System.out.println("Available Tags:");
				int tagCounter = 1;
				List<Tag> tagList = new ArrayList<>(loggedUser.getTags());
				for (Tag tag : tagList) {
					System.out.println(tagCounter + ". " + tag.getName());
					tagCounter++;
				}
				System.out.print("Enter tag index to apply: ");
				int tagIndex = sc.nextInt() - 1;
				sc.nextLine();
				if (tagIndex < 0 || tagIndex >= tagList.size()) {
					System.out.println("Invalid tag index.");
					break;
				}
				Tag selectedTag = tagList.get(tagIndex);
